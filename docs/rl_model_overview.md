...
終端スコアがチーム勝敗を教え、補助報酬が「場外へ出続けない」「敵ミサイルを振り切る」「自分で撃つ・撃墜する」「自分が撃墜されない」という中間目標を教える構造です。
...
1. 前ステップのミサイル一覧と現ステップのミサイル一覧を `(teamId, targetFighterIndex, age)` で突き合わせる。`age` はシミュレータの `deltaTime` ずつ単調増加するので、`age_curr ≈ age_prev + dt` の最小差分マッチで同一ミサイルを特定する。
