...
| 接近ミサイルペナルティ | 最大 `-0.10` / missile / step | 半径 `100` 以内の敵ミサイルが自機へ向かっているほど本人へ減点 |
| ミサイル発射報酬 | `0.03` | 有効にミサイルを発射できた Blue 機本人へ加点 |
| 撃墜報酬 | `1.0` | Red を撃墜した Blue 機本人へ加点 |
| 自機損失ペナルティ | `-5.0` | 前 step 生存、今 step 非生存になった Blue 本人へ減点 |

終端スコアがチーム勝敗を教え、補助報酬が「場外へ出続けない」「危険な敵ミサイルから離れる」「自分で撃つ・撃墜する」「自分が撃墜されない」という中間目標を教える構造です。

撃墜報酬 `1.0` と自機損失ペナルティ `-5.0` をあえて非対称にしているのは、固定 Red AI に対しては攻撃側の credit assignment が容易で「殺す」方向に勾配が偏りやすいためです。「死なない」ほうを強く重視させることで、ミサイル回避行動が学習されやすくなります。
接近ミサイルペナルティを `-0.10` まで強めているのも同じ理由で、終端の死亡ペナルティだけに頼らず、危険なミサイルが近づいている間ずっと連続的なシグナルが乗るようにしています。
